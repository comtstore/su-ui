  ...
  return useObserver(() => {
    const isActiveBefore =
      props.index === props.items.length - 1
        ? false
        : props.active.findIndex(
            (item) =>
              item[props.itemKey] ===
              props.items[props.index + 1][props.itemKey],
          ) !== -1;
    const isActiveAfter =
      props.index === 0
        ? false
    ...
    return (
      <div
        className={cx("su-tab-bar-item", {
          "su-tab-bar-item__active":
            props.active.findIndex(
              (item) => item[props.itemKey] === props.item[props.itemKey],
            ) !== -1,
