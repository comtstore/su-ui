...
function MenuList(props: PaginationProps) {
  ...
  const pageNum = useMemo(
    () => Math.ceil(props.num / props.limit),
    [props.num, props.limit],
  );

  return (
    <div className={cx("su-pagination", props.className, props.classes?.root)}>
      <div className="su-pagination-main">
        ...
        {1 < props.page - Math.floor(props.pageLimit / 2) ? (
          <div className="su-pagination-main-page-more">···</div>
        ) : null}
        {new Array(pageNum)
          .fill(1)
          .map((_, index) => index + 1)
          .map((currentPageNum) => {
            if (
              currentPageNum < props.page - Math.floor(props.pageLimit / 2) ||
              currentPageNum >=
                props.page - Math.floor(props.pageLimit / 2) + props.pageLimit
            )
              return null;
            return (
              <div
