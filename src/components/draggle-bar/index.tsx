...
function DraggleBar(props: DraggleBarProps) {
  const [controller] = useState(() => new DraggleBarController());
  useEffect(() => {
    controller.updateProps(props);
  }, [props]);

  useEffect(() => {
    controller.initialGroup(props.group);
  }, [props.group]);
  ...
  return useObserver(() => (
    <div
      className={cx("draggle-line", props.classes?.root, props.className)}
      style={{
        ...props.style,
        ...props.styles?.root,
      }}
    ...
    >
      {controller.isDragging ? <div className="line" /> : null}
    </div>
  ));
}
