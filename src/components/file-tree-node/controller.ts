...
class FileTreeNodeController extends baseController<FileTreeNodeProps> {
  ...
  public get isDisabled(): boolean {
    return (
      this.disabled.findIndex(
        (item) =>
          item[this.props.itemKey] === this.props.item[this.props.itemKey],
      ) !== -1 ||
      (!!this.props.disableLeaf &&
        this.props.item[this.itemType] !== this.folderType)
    );
  }
  ...
  public handleArrowClick = () => {
    ...
    if (this.props.shouldOpenChange) {
      const shouldChange: boolean | Promise<boolean> =
        this.props.shouldOpenChange(this.props.item, this.isOpen);
