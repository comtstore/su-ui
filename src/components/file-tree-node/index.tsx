...
function FileTreeNode(props: PropsWithChildren<FileTreeNodeProps>) {
    ...
    return (
      <>
        {props.item[controller.itemType] === controller.folderType &&
          (controller.isOpen ? (
            <FolderOpenIcon
              sx={{ fontSize: 25, color: "var(--su-tc5)" }}
            ></FolderOpenIcon>
          ) : (
            <FolderIcon
              sx={{ fontSize: 25, color: "var(--su-tc5)" }}
  ...
  const renderRoot = () => {
    return (
      <div
        className={cx("su-file-tree-node__root", {
          [props.classes?.active ?? "su-file-tree-node__active"]:
            controller.isActive && !controller.isDisabled,
          [props.classes?.disabled ?? "su-file-tree-node__disabled"]:
            controller.isActive && controller.disabled,
  ...
  return useObserver(() => (
    ...
    >
      ...
      controller.isOpen ? (
        <div>{renderChildrenNodes(props.item[controller.itemChildren])}</div>
      ) : null}
    </div>
  ));
}
