          ...
          return (
            <div
              className={cx({
                ...
                vertical: props.direction === "vertical",
                "su-menu-list-item__active":
                  props.active &&
                  (props.active === menuListItem ||
                    (props.active instanceof Array &&
                      props.active.findIndex(
                        (item) =>
                          getMenuListItemKey(item, props) ===
                          getMenuListItemKey(menuListItem, props),
                      ) !== -1)),
              })}
              key={"item-" + index}
            ...
            >
              {menuListItem.icon ? (
                <div className="su-menu-list-item-icon">
                  {props.iconComponent
                    ? props.iconComponent(menuListItem.icon, menuListItem)
                    : menuListItem.icon}
                </div>
              ) : null}
              <div
              ...
              >
                {getMenuListItemLabel(menuListItem, props)}
              </div>
            </div>
          );
        }
